...
def predict(TestX, test_y, gcnnodes = 4, features = 5, fcnnodes = 64, dropout = 0., lr = 1e-3, name = "", l1 = 0.):
    ...
    checkpoint_path = "GCN_record_pos_dist%.2f_2/training_mgcn_%0.2f_%s_%s_hd%d_fc%d/cp-150.ckpt" \
                      % (rad, r, name, dropout, gcnnodes, fcnnodes)
    # val_loss = np.load("val_epoch_%d_%d_%d.npy" % (0, fcnnodes, gcnnodes))
    # checkpoint_path = "GCN_record_pos_dist%.2f_2/training_mgcn_%0.2f_%s_%s_hd%d_fc%d/cp-%02d.ckpt"% (rad, r, name, dropout, gcnnodes, fcnnodes, val_loss[0])
