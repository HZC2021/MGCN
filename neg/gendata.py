...
for regions in range(3,15):
    ...
    if regions == 10:
        ...
        for i in range(regions):
            ...
            #np.save(r".\data\pos_regions%d_group%d.npy" % (regions, i), group)
            groups_pos[i] = group
        heatmap.save('Kmeans_pos_%d.html'%regions)
#np.savetxt("data\\sil_scores_pos.txt", sil_scores)
...
np.savetxt("data_neg\\sil_scores_neg.txt", sil_scores)
...
# save index
f = open("./data_neg/test_idx.pkl", "wb")
pickle.dump(test_idx, f)
f.close()
f = open("./data_neg/train_idx.pkl", "wb")
...
for rad in radius_ratios:
    ...
    for r in range(len(ratios)):
        ...
        f = open("./data_neg/dist_%.2f/ext_%.2f/1/train_X.pkl"%(rad, ratios[r]), "wb")
        pickle.dump(train_X, f)
        f.close()
        f = open("./data_neg/dist_%.2f/ext_%.2f/1/train_idx.pkl"%(rad, ratios[r]), "wb")
        pickle.dump(train_idx_ext, f)
        f.close()
        f = open("./data_neg/dist_%.2f/ext_%.2f/1/train_A.pkl" %(rad, ratios[r]), "wb")
        pickle.dump(train_A, f)
        f.close()
        np.save("./data_neg/dist_%.2f/ext_%.2f/train_y.npy" %(rad, ratios[r]), train_label)
        ...
        np.save("./data_neg/gcn/dist_%.2f/ext_%.2f/1/train_GCN_X.npy" % (rad, ratios[r]), train_GCN_X)
        np.save("./data_neg/gcn/dist_%.2f/ext_%.2f/1/train_GCN_A.npy" % (rad, ratios[r]), train_GCN_A)
        np.save("./data_neg/gcn/dist_%.2f/ext_%.2f/train_GCN_y.npy" % (rad, ratios[r]), train_GCN_label)
    ...
    f = open("./data_neg/dist_%.2f/1/test_X.pkl"%(rad), "wb")
    pickle.dump(test_X, f)
    f.close()
    f = open("./data_neg/dist_%.2f/1/test_A.pkl"%(rad), "wb")
    pickle.dump(test_A, f)
    f.close()
    np.save("./data_neg/dist_%.2f/1/test_y.npy"%(rad), test_label)
    ...
    np.save("./data_neg/gcn/dist_%.2f/1/test_GCN_X.npy" % (rad), test_GCN_X)
    np.save("./data_neg/gcn/dist_%.2f/1/test_GCN_A.npy" % (rad), test_GCN_A)
    np.save("./data_neg/gcn/dist_%.2f/1/test_GCN_y.npy" % (rad), test_GCN_label)
