...
def train(TrainX, train_y, TestX, test_y, gcnnodes = 4, features = 5, fcnnodes = 64, dropout = 0., lr = 1e-3, name = "", l1 = 2.5e-4, epochs = 150):
    ...
    checkpoint_path = "GCN_record_neg_dist%.2f_2/training_mgcn_%0.2f_%s_%s_hd%d_fc%d/cp-{epoch:02d}.ckpt" \
...
if __name__ == "__main__":
    ...
    for i in trainid:
        total.remove(i)
    np.save("data_neg/dist_%.2f/ext_0.50/validx_gcn.npy" % rad, np.array(total))
